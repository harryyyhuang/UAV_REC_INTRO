...
def build_args():
    ...
    parser.add_argument('--base_dir', required=True, type=Path) ## ground true data
...
def visualize_seg_argmax(args, viz):
    ...
    vertex_label_weight=np.zeros((scene_mesh_vertices.shape[0], 41))
    ...
    for text_id in range(1, len(os.listdir(seg_image_path)), args.skip_nums):
        ...
        # array that we can further calculate maximum posterior probability label, but let's
        # leave it in the next intro
        scene_mesh, vertex_label_weight = util.label_argmax_mesh(scene_mesh,
                                                         depth_img, label_img, lines,
                                                         vertex_projection, vertex_camera_coord,
                                                         viz, text_id, vertex_label_weight)
        
        # print(vertex_label_weight[3])
        # to make the back-projection progress more easy to observe
        # we specify the viewing extrinsic
        viz_extrinsic = np.array([[0.1589852858550839, 0.81266088242727896, -0.56062997516552193, 0.0],
                                  [0.98722197737634287, -0.12465113670926495, 0.099271655079542942, 0.0],
...
if __name__ == '__main__':
    ...
    elif(args.visualize_type == "seg_argmax_mesh"):
        visualize_seg_argmax(args, viz)
